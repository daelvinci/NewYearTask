...
namespace ConsoleApp1
{
    internal class Employee
    {
        ...
        public static bool CheckFullName(string fullName)
        {
            if (!string.IsNullOrWhiteSpace(fullName))
            {
                ...
                if (splitted.Length != 2)
                {
                    try
                    {
                        throw new Exception(" Fullname'i yalniz 2 sozden ibaret ola biler ve Sozler arasinde yalniz 1 bosluq qoyun");
                    }
                    catch (Exception e)
                    {
                        Console.WriteLine($"\n{e.Message}\n");
                ...
                else
                {
                    if (splitted[0].Length > 1 && splitted[1].Length > 1)
                    {
                        ...
                        {
                            if (HasSymbolOrDigitOrUpper(splitted[i]))
                            {
                                try
                                {
                                    throw new Exception(" Yalniz kicik herfler daxil oluna biler");
                                }
                                catch (Exception e)
                                {
                                    Console.WriteLine($"\n{e.Message}\n");
        ...
        public static bool HasSymbolOrDigitOrUpper(string input)
        {
